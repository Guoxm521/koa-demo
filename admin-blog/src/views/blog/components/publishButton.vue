          ...
          <div class="form-item">
            ...
            <div class="content">
              <div
                ...
                :key="index"
                :class="['item', form.category == item.id ? 'active' : '']"
              >{{ item.name }}</div>
            </div>
          </div>
          <div class="form-item">
          ...
          <div class="form-item">
            ...
            <div class="content">
              <div v-show="!form.cover">
                ...
                <input type="file" ref="upload" @change="changeImgae" style="display: none" />
              </div>
              <div class="image_box" v-show="form.cover">
                <el-icon :size="20" class="delete_icon" @click="deleteCover">
                  <delete />
                </el-icon>
                <el-image style="width: 160px; height: 106px" :src="form.cover" fit="cover" />
              </div>
            </div>
          </div>
          <div class="form-item">
...
import { reactive, ref, toRef, defineProps, watch, defineEmits } from "vue"
...
watch(props, (newProps, oldProps) => {
  if (newProps.title !== form.title) {
    form.title = newProps.title
  }
  if (newProps.content !== form.content) {
...
const submit = async () => {
  ...
  if (!form.category) return notice("请选择文章所属分类")
...
if (route.query && route.query.id) {
  ...
  getBlogDetail({ id: route.query.id }).then((res: any) => {
    if (res.code === 200) {
      form.category = res.data.category
      form.tag = res.data.tag ? res.data.tag.split(',').map(e => Number(e)) : []
      form.des = res.data.des
      form.cover = res.data.cover
      form.status = 0
      form.title = res.data.title
      form.content = res.data.content
      emits('getcontent', { content: form.content, title: form.title })
    }
  })
}
</script>
